...
<div class="container mx-auto py-8">
    <h1 class="text-3xl font-bold mb-6 text-center">Daftar Harga Sampah Bank Sampah Pusat</h1>
        ...
        <table id="dataTable" class="min-w-full table-auto">
            ...
            <tbody class="text-gray-700">
                ...
                <tr class="border-b hover:bg-gray-100">
                    <td class="px-4 py-2 border">{{ $wastePrice->waste_id }}</td>
                    <td class="px-4 py-2 border">{{ $wastePrice->waste->kategori }}</td>
                    <td class="px-4 py-2 border">{{ $wastePrice->waste->jenis }}</td>
                    <td class="px-4 py-2 border">{{ $wastePrice->price }}</td>
                    <td class="px-4 py-2 border">
                        <button class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded" onclick="openEditModal('{{ $wastePrice->id }}')">
                            <i class="fas fa-edit"></i>
                        </button>
                        <form action="{{ route('admin.daftar_harga_sampah_pusat.destroy', $wastePrice->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                        ...
                        <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" onclick="showInfoModal('{{ $wastePrice->waste_id }}')">
...
<div id="addModal" class="hidden fixed z-10 inset-0 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            ...
            <div class="mt-2">
                <form action="{{ route('admin.daftar_harga_sampah_pusat.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-4">
...
@foreach($wastePrices as $wastePrice)
<div id="editModal-{{ $wastePrice->id }}" class="hidden fixed z-10 inset-0 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 transition-opacity" aria-hidden="true">
            <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
        </div>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            ...
            <div class="mt-2">
                <form action="{{ route('admin.daftar_harga_sampah_pusat.update', $wastePrice->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="waste_id" class="block text-sm font-bold mb-2">Jenis Sampah:</label>
                        <select id="waste_id" name="waste_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                            @foreach($dataSampahs as $sampah)
                            <option value="{{ $sampah->id }}" {{ $wastePrice->waste_id == $sampah->id ? 'selected' : '' }}>{{ $sampah->jenis }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        <label for="price" class="block text-sm font-bold mb-2">Harga per Kg:</label>
                        <input type="number" step="0.01" id="price" name="price" value="{{ $wastePrice->price }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    </div>
                    <div class="flex justify-end">
                        <button type="button" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2" onclick="closeModal('editModal-{{ $wastePrice->id }}')">Batal</button>
...
@foreach($wastePrices as $wastePrice)
<div id="infoModal-{{ $wastePrice->waste_id }}" class="hidden fixed z-10 inset-0 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 transition-opacity" aria-hidden="true">
            <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
        </div>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            ...
            <div class="mt-2">
                <p>{{ $wastePrice->waste->sale_unit }}</p>
                <div class="flex justify-end mt-4">
                    <button type="button" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded" onclick="closeModal('infoModal-{{ $wastePrice->waste_id }}')">Tutup</button>
                </div>
            </div>
        </div>
    </div>
