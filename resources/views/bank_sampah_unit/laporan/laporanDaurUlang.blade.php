    ...
    <div class="w-full">
        <div class="bg-white mt-5 h-fit mx-10 rounded-md p-5">
            <div class="flex justify-between gap-2">
                <form id="filter-form" action="{{ route('daur_ulang.index') }}" method="GET">
...
@section('scripts')
    ...
    <script>
        document.getElementById('export-pdf').addEventListener('click', function() {
            ...
            let url = `{{ route('daur_ulang.exportPdf') }}?tahun=${tahun}&bulan=${bulan}`;
            window.location.href = url;
        });
        document.getElementById('export-excel').addEventListener('click', function() {
            let tahun = document.getElementById('tahun-filter').value;
            let bulan = document.getElementById('bulan-filter').value;
            let url = `{{ route('daur_ulang.exportExcel') }}?tahun=${tahun}&bulan=${bulan}`;
            window.location.href = url;
        });
    </script>
@endsection
