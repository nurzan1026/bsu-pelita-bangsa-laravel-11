        ...
        <div class="bg-white mt-5 h-fit mx-10 rounded-md p-5">
            <div class="flex justify-end gap-2">
                <button class="bg-red-500 hover:bg-red-700 font-bold py-1.5 px-2 rounded">
                    <a href="{{ route('rekap.export.pdf') }}" class="flex items-center">
                ...
                <button class="bg-green-500 hover:bg-green-700 font-bold py-1.5 px-2 rounded">
                    <a href="{{ route('rekap.export.excel') }}" class="flex items-center">
                        <svg class="w-6 h-6 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                            height="24" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @foreach ($rekapBulanan as $bulan => $data)
                        @if ($data['total_kg'] !== '-')
                            <tr>
                                ...
                                <td class="py-2 px-4 border-b">Rp
                                    {{ $data['total_rp'] == '-' ? '-' : number_format($data['total_rp'], 0, ',', '.') }}
                                </td>
                            </tr>
                        @else
                            <tr>
