    ...
    <div class="w-full">
        <div class="bg-white mt-5 h-fit mx-10 rounded-md p-5">
            <div class="flex justify-between gap-2">
                <form id="filter-form" action="{{ route('penjualan.laporan') }}" method="GET">
            ...
            <table class="min-w-full bg-white border border-gray-200 mt-4">
                <thead class="bg-gray-300">
                    <tr>
                        ...
                        <th class="border border-gray-200 px-4 py-2">Tanggal Penjualan</th>
                        <th class="border border-gray-200 px-4 py-2">Jenis Sampah</th>
                        <th class="border border-gray-200 px-4 py-2">Berat Sampah (kg)</th>
                        <th class="border border-gray-200 px-4 py-2">Harga per Kg</th>
                    ...
                    @foreach ($penjualans as $penjualan)
                        @php
                            ...
                            foreach ($penjualan->detailPenjualans as $detail) {
                                // Konversi harga dari string ke float sebelum melakukan perhitungan
                                $hargaPerKg = (float) str_replace(',', '', $detail->sampah->harga->price ?? '0');
                                $totalHarga += $hargaPerKg * $detail->berat;
                            }
                        @endphp
                        @foreach ($penjualan->detailPenjualans as $detailIndex => $detail)
                            <tr>
                                @if ($detailIndex === 0)
                                    <td class="border border-gray-200 px-4 py-2"
                                        rowspan="{{ $penjualan->detailPenjualans->count() }}">
                                        {{ $rowIndex++ }}
                                    </td>
                                    <td class="border border-gray-200 px-4 py-2"
                                        rowspan="{{ $penjualan->detailPenjualans->count() }}">
                                ...
                                <td class="border border-gray-200 px-4 py-2">Rp
                                    {{ number_format((float) str_replace(',', '', $detail->sampah->harga->price ?? '0'), 2, ',', '.') }}
                                </td>
                                @if ($detailIndex === 0)
                                    <td class="border border-gray-200 px-4 py-2"
                                        rowspan="{{ $penjualan->detailPenjualans->count() }}">
...
@section('scripts')
    ...
    <script>
        document.getElementById('export-pdf').addEventListener('click', function() {
            ...
            let url = `{{ route('penjualan.exportPdf') }}?tahun=${tahun}&bulan=${bulan}`;
        ...
        document.getElementById('export-excel').addEventListener('click', function() {
            let tahun = document.getElementById('tahun-filter').value;
            let bulan = document.getElementById('bulan-filter').value;
            let url = `{{ route('penjualan.exportExcel') }}?tahun=${tahun}&bulan=${bulan}`;
            window.location.href = url;
        });
    </script>
@endsection
