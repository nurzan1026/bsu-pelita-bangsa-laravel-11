...
@section('content')
    <div class="w-full px-4 md:px-10 text-2xl md:text-3xl font-bold tracking-wider mt-14">
        <div class="w-full md:w-fit pb-2 flex items-center gap-2">
            ...
            <h1 class="text-center md:text-left">Nasabah</h1>
        </div>
    </div>
    <div class="w-full px-4 md:px-10">
        <div class="bg-white mt-5 h-fit rounded-md p-5">
            <div class="flex flex-col md:flex-row justify-between mb-3 w-full">
                <form action="" class="flex items-center mb-4 md:mb-0">
                    <input type="search" id="search-input" placeholder="Cari nasabah..."
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white border border-gray-200">
                    <thead class="bg-gray-300">
                        <tr>
                            <th class="py-2 px-4 border-b text-xs md:text-sm">No</th>
                            <th class="py-2 px-4 border-b text-xs md:text-sm">Nama</th>
                            <th class="py-2 px-4 border-b text-xs md:text-sm">Nomor Induk</th>
                            <th class="py-2 px-4 border-b text-xs md:text-sm">Alamat</th>
                            <th class="py-2 px-4 border-b text-xs md:text-sm">Aksi</th>
                        </tr>
                    </thead>
                    <tbody id="nasabah-table-body">
                        @foreach ($nasabahs as $index => $nasabah)
                            <tr class="hover:bg-gray-100">
                                <td class="py-2 px-4 border-b text-xs md:text-sm">{{ $loop->iteration }}</td>
                                <td class="py-2 px-4 border-b text-xs md:text-sm">{{ $nasabah->nama }}</td>
                                <td class="py-2 px-4 border-b text-xs md:text-sm">{{ $nasabah->nomor_induk }}</td>
                                <td class="py-2 px-4 border-b text-xs md:text-sm">{{ Str::limit($nasabah->alamat, 50) }}
                                </td>
                                <td class="py-2 px-4 border-b text-xs md:text-sm">
