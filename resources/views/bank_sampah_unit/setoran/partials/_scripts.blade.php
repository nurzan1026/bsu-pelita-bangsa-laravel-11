    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        addFieldButton.addEventListener('click', function() {
            ...
            <div class="w-1/2">
                ...
                <select id="jenis-sampah-${fieldCount}" name="detail_setoran[${fieldCount}][waste_id]" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2" required>
                        ...
                        fetch(`/setoran/${id}`, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    Swal.fire('Error!',
                                        'Terjadi masalah saat menghapus setoran.',
                                        'error');
                                }
                            });
                    }
                    ...
                    .then(data => {
                        ...
                        data.setoran.detail_setoran.forEach((detail, index) => {
                            ...
                            <div class="w-1/2">
                                ...
                                <select id="edit-jenis-sampah-${index}" name="detail_setoran[${index}][waste_id]" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2" required>
                                    ${data.sampah.map(sampah => `<option value="${sampah.id}" ${sampah.id === detail.waste_id ? 'selected' : ''}>${sampah.jenis}</option>`).join('')}
                                </select>
                            </div>
                            <div class="w-1/2 ml-4">
        ...
        searchInput.addEventListener('input', function() {
            ...
            fetch(`/setoran/search?query=${query}`)
                ...
                .then(data => {
                    ...
                    tableBody.innerHTML = ''; // Kosongkan isi tabel sebelum menambahkan hasil baru

                    data.forEach((item, index) => {
                        const totalBerat = item.detail_setoran.reduce((sum, detail) => sum +
                            detail.berat, 0);
                        ...
                        <td class="border border-gray-200 px-4 py-2">${totalBerat} kg</td>
                    ...
                    `;
                        ...
                        // Attach event listeners for the newly added rows
        ...
        function attachEventListenersToRow(row) {
            ...
            const seeButton = row.querySelector('.see-button');
            ...
            if (editButton) {
                editButton.addEventListener('click', function() {
                    ...
                    fetch(`/setoran/${id}/edit`)
                        .then(response => response.json())
                        .then(data => {
                            // Isi form edit berdasarkan data yang diambil
                            editNasabahInput.value = data.setoran.nasabah_id;
                            editNasabahNama.value = data.setoran.nasabah.nama;
                            editTanggal.value = data.setoran.tanggal_setor;
                            editForm.action = `/setoran/${id}`;
